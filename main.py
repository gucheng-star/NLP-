...
# 超参数设置
data_path =  os.path.join(os.path.dirname(__file__), 'data', 'data.txt')              # 数据集
vocab_path = os.path.join(os.path.dirname(__file__), 'data', 'vocab.pkl')             # 词表
save_path = os.path.join(os.path.dirname(__file__), 'saved_dict', 'lstm.ckpt')        # 模型训练结果
embedding_pretrained = \
    torch.tensor(
    np.load(
        os.path.join(os.path.dirname(__file__), 'data', 'embedding_Tencent.npz'))
    ["embeddings"].astype('float32'))
                                            # 预训练词向量
embed = embedding_pretrained.size(1)        # 词向量维度
dropout = 0.5                               # 随机丢弃
num_classes = 2                             # 类别数
num_epochs = 30                             # epoch数,训练次数
...
def plot_acc(train_acc):
    ...
    plt.ylabel('Acc')
    plt.legend(loc='best')
    plt.savefig(os.path.join(os.path.dirname(__file__), 'results', 'acc.png'), dpi=400)
...
def plot_loss(train_loss):
    ...
    plt.ylabel('loss')
    plt.legend(loc='best')
    plt.savefig(os.path.join(os.path.dirname(__file__), 'results', 'loss.png'), dpi=400)
...
def train( model, dataloaders):
    ...
    for i in range(num_epochs):
        ...
        if early_stop_counter >= patience:
            print(f"早停机制在 {i+1} 轮训练后触发，因为开发集损失没有改善。")
            break
        ...
        print(f"Epoch {i+1} 所用时间: {epoch_time_dif}")
...
def result_test(real, pred):
    ...
    plt.savefig(os.path.join(os.path.dirname(__file__), 'results', 'reConfusionMatrix.tif'), dpi=400)
